...
### Signal alerts (Discord, 4H BUYs) — BUILT, ON HOLD (2026-09-26, owner: "hold off on this")
**Raised**: owner, 2026-09-26 — "alert me when any of my tokens on watchlist hit a buyzone on the 4h candle for any of the signals we've built". **Status**: built and tested, not released. The code is the local branch **`hold/signal-alerts`** (commit `aebd967`, on top of `dc861e9`; local only — not pushed, so Vercel doesn't build a preview against missing tables).

Decided: BUY signals only, all five indicators, 4H; Discord webhook per user (Settings → Signal alerts; stored encrypted; only discord.com webhook URLs); each signal sent once (`signal_alerts_sent`); a 12 h catch-up window; messages carry the "unvalidated research signal — not trading advice" caveat. Scheduler: **Supabase pg_cron + pg_net** calling `/api/cron/signal-alerts` (CRON_SECRET) at `2 */4 * * *` — Vercel Hobby crons are daily-only. Dry run on the owner's watchlist: 13 BUYs in 3 days (mostly SMC flips at 12H boundaries); a warm run takes ~6 s.

To release: (1) owner runs the two `create table` blocks at the end of `db/schema.sql` on the branch; (2) rebase `hold/signal-alerts` onto main, full gate incl. build, `git merge --ff-only`, push; (3) owner runs the pg_cron block (fills in CRON_SECRET → Vault, and the production URL); (4) owner saves a webhook in Settings and sends a test; (5) check the first scheduled run's `last_status`. Also check production `NEXT_PUBLIC_SITE_URL` is the real domain (the chart links use it). Worth deciding first: the pullback-indicator backtest (above) hasn't run, so these alerts would be for unvalidated signals.
...
### DeFi coverage gaps found against DeBank (2026-09-25)
- **INIT Capital on Blast**: the adapter (`adapters/initCapital.ts`) covers Mantle only. Blast's pools hold a rebasing wrapper (WWETH, WUSDB; `rebaseWrapperAddress` in `app.init.capital/static/json/pools.json`) whose unwrap to ETH/USDB isn't verified. DefiLlama's adapter calls `wrapper.toAmt`; verify that live before adding Blast (PosManager `0xA0e172f8BdC18854903959b8f7f73F0D332633fe`).
- **Receipt tokens CoinGecko doesn't list** (Taiko `aTkoWETH` ~$13, Blast Hyperlock `hUSDB`/expired Pendle PT ~$15 on Metamask Main): the EVM scan only checks tokens on CoinGecko's list, so it never sees them. Proposed: discover them from an indexer (Zerion's wallet list, or chain explorers where Zerion doesn't cover the chain) and value them as their underlying via `adapters/receiptTokens.ts`.
- **Chains still unscanned**: Aurora, Sonic, DBK Chain (small balances on Metamask Main). BNB Chain's gap ($41 DeBank vs $17) is unexplained: Etherscan's free tier doesn't cover BNB.
