...
### CoinGecko: one key deep until the monthly reset (2026-10-01) — and a cap = a lost day, not a degraded one
**Raised**: Phase 3a live run, 2026-09-23. **Status**: open, time-boxed. The primary Demo key is at its 10,000 calls/month cap (error 10006; hit 2026-09-22 by the full backfill). The whole app now runs on `COINGECKO_API_KEY_BACKUP`, with nothing behind it.
