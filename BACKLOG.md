...
- **What**: 3a–3c shipped 2026-09-25 (`9e94c78`, `eb99d08`, `948f282`). Nothing reads or writes the legacy stores anymore. To drop: the `prices` table, `coin_market_data`, `exchange_asset_registry`, `token_registry`'s price/stat columns (`price_usd`, `price_at`, `change_24h_pct`, `change_1h_pct`, `change_7d_pct`, `change_30d_pct`, `market_cap`), `coin_cache.usd`, and `holdings.coingecko_id`. **Check first**: `holdings.coingecko_id` is still the key input for `auto_cosmos` and `manual_qty` rows in `resolvePriceKey` and the Cosmos carry-forward columns, so it probably stays; re-check before dropping.
