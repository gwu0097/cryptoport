...
// Plain FormData-less args and throw-on-error, same split as
// settings/actions.ts vs ../actions.ts — this isn't wired through
// useActionState, so it follows the throw convention rather than
// ../(auth)/walletActions.ts's {ok, error} one, even though both are
// called the same imperative way from WalletButton.tsx.
export async function requestWalletLink(chain: WalletChain, address: string, chainId?: number): Promise<string> {
  await requireUser();
  return createChallenge(chain, address, "link", chainId);
}
...
// ignores the return value and just refreshes in place.
export async function completeWalletLink(signatureHex: string): Promise<{ walletId: string }> {
  await requireUser();
  ...
  if (!already.data?.length) {
    // user_id isn't set explicitly — this runs under a real session, so the
    // column's default auth.uid() applies, same as createWallet/resolveTagId
    // elsewhere in this app. RLS's own with check (user_id = auth.uid())
    // structurally prevents this from ever writing a row for anyone else.
    ...
    if (error) {
      if (error.code === "23505") {
        ...
        // doesn't say whose — that would leak whether a given address has
