...
- **UI name vs internal name (decided 2026-09-23).** Users see this as **"Fundamentals"**: the sidebar entry, the page heading, the browser tab titles and user-facing copy. "Screener" implies it screens *for* something, and Phase 4 found it doesn't. **Internally it stays "screener"**: the `/screener` route, every `screener_*` table, `src/lib/screener/`, `src/components/screener/`, the `screener-snapshot` cron, scripts and these docs. It's a label change, not a refactor. Renaming live tables, a deployed cron path and a documented history would add real risk for no benefit to anyone using the page. When the two names appear together, the UI word is "Fundamentals" and the code/DB word is "screener".
- **Default view** (`/screener`, labeled "Fundamentals" in the sidebar's Research group since 2026-09-23): a research table of every rated asset, all columns sortable, **default sort: annualized revenue**, with an option to include unrated assets with the filters they failed. The regime panel stays (descriptive market context). The "Unvalidated" banner and the latest backtest's caption stay.
