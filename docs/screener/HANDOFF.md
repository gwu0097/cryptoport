...
- **Closed 2026-09-23:** decision 2. "Top N by grade" for Phase 5 is **rejected**: the Phase 4 backtest found no predictive value for the ranking. Phase 5 is rescoped to research notes on user-picked or all Pass-tier assets that never rank or recommend (SPEC, Phase 5 amendment). The product is now a verified research dataset with a risk filter, not a signal (SPEC, "Product").
...
2. **~~Phase 5's "top N by Fundamental score" needs a real replacement definition.~~ CLOSED 2026-09-23: rejected; see the status update above.** That score doesn't exist anymore (Quality & Risk is a tier now, Score B/momentum is primary) — the obvious candidate is "top N by Timing grade among Pass-tier assets," but that's my inference, not a confirmed decision. Needs an explicit call when Phase 5 is reached.
